...
export const CreateTableForm = ({ onSuccess, onClose }: CreateTableFormProps) => {
    ...
    const { getInputProps, handleSubmit } = useForm({
        ...
        validate: (values: any) => {
            ...
            if (!values.capacity)
                errors.price = 'Debes especificar la capacidad'
            return errors
        },
...
export const UpdateTableForm = ({ table: { id, capacity, sortId, name }, onUpdate, onClose }: UpdateTableFormProps) => {
    ...
    const onDelete = async () => {
        if (confirm(`¿Estas seguro de eliminar la mesa ${name}?`))
    ...
    const { getInputProps, handleSubmit } = useForm({
        ...
        validate: (values: any) => {
            ...
            if (!values.capacity)
                errors.price = 'Debes especificar la capacidad'
            if (values.sortId && values.sortId < 0)
                errors.sortId = 'El indice no puede ser negativo'
            return errors
        },
...
export const CreateActivityForm = ({ table: { id, capacity }, onSuccess, onClose }: CreateActivityFormProps) => {
    ...
    const { getInputProps, handleSubmit } = useForm({
        ...
        validate: (values: any) => {
            const errors: { [key: string]: string } = {}
            if (!values.people)
                errors.people = 'Debes especificar la gente a ocupar la mesa'
            return errors
        },
